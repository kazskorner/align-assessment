...
/**
 * ALIGN Results — Tier B
 * Design: matches landing page (dark theme, blue-cyan accents, Plus Jakarta Sans)
...
// ── STYLES ────────────────────────────────────────────────────────────────────
const ACCENT = '#7ec8e3'; // Tier B uses a softer blue accent
...
const S = {
  ...
  tierBadge: {
    ...
    fontSize: 12, fontWeight: 600, color: 'rgba(126,200,227,0.95)',
  ...
  personaCard: {
    background: `linear-gradient(135deg,rgba(126,200,227,0.06),transparent)`,
  ...
  personaQuote: {
    ...
    fontSize: 200, fontWeight: 700, color: `rgba(126,200,227,0.05)`,
  ...
  ctaBody: { fontSize: 16, color: '#767676', maxWidth: 480, margin: '0 auto 36px', lineHeight: 1.7, position: 'relative' },
  ctaBtn: {
    ...
    <div style={S.page}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:700px){.grid2{grid-template-columns:1fr!important}.ctaBand{margin:0 20px 60px!important;padding:48px 24px!important}.traitCard{padding:32px 24px!important}.personaCard{padding:36px 28px!important}.section{padding:60px 20px!important}}
      `}</style>
      ...
      <section style={S.hero}>
        ...
        <div style={S.overlay} />
        <div style={{ position:'absolute',inset:0,zIndex:1,pointerEvents:'none',background:`radial-gradient(ellipse 80% 50% at 50% 100%,rgba(126,200,227,0.08) 0%,transparent 65%)` }} />
        ...
        <div style={{ position:'absolute',bottom:40,left:'50%',transform:'translateX(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:10,zIndex:4 }}>
          ...
          <div style={{ width:1,height:44,background:`linear-gradient(to bottom,rgba(126,200,227,0.6),transparent)` }} />
        </div>
      </section>
      ...
      {/* ── CTA — TIER B: Discovery Call ── */}
      <div style={S.ctaBand} className="ctaBand">
        <div style={S.ctaGlow} />
        <div style={{ ...S.sectionTag, justifyContent:'center', marginBottom:20 }}>Your Next Step</div>
        <h2 style={S.ctaTitle}>
          You're closer than you think.<br />
          <span style={{ color: ACCENT }}>Let's map the path forward.</span>
        </h2>
        <p style={S.ctaBody}>
          Your results reveal a clear direction. A single discovery call is where we take that direction and build it into something real.
        </p>
